...
class ContentExtractor:
    ...
    BOILERPLATE_PATTERNS = [
        'navbar', 'navigation', 'menu-', 'sidebar', 'side-bar',
        'footer', 'site-footer', 'page-footer',
        'header-nav', 'site-header',
        'advertisement', 'ad-', 'cookie-', 'popup', 'modal', 'banner-ad',
    ...
    def extract_main_content(soup: BeautifulSoup) -> BeautifulSoup:
        ...
        main_content = (
            soup.find('main') or
            soup.find('div', class_=re.compile(r'page.*content|main.*content|site.*content|container.*page', re.I)) or
        ...
        for element in list(main_content.find_all(['nav', 'footer', 'aside'], recursive=False)):
            if element:
                try:
                    element.decompose()
                except:
    ...
    def extract_and_clean(cls, html: str, url: str) -> Dict[str, any]:
        ...
        has_code_blocks = bool(
            main_content.find('code') or
            main_content.find('pre') or
            '```' in body_text
        )
...
class ContentEnricher:
    ...
    @staticmethod
    def classify_content_type(url: str, title: str, body_text: str, link_density: float) -> str:
        ...
        if len(body_text) > 1500 and any(word in title_lower for word in ['tutorial', 'guide', 'how to']):
            return 'tutorial'
        
        return 'article'
    
    @classmethod
    def enrich(cls, extracted_data: Dict, url: str, http_status: int, crawl_depth: int) -> AIDocument:
...
class WebCrawler:
    ...
    def crawl(self) -> List[AIDocument]:
        ...
        while self.url_queue and len(self.documents) < self.max_pages:
            ...
            logger.info(f"Crawling [{len(self.documents) + 1}/{self.max_pages}] depth={depth}: {current_url}")
